...
class Encoder(nn.Module):
    def __init__(self, in_channels, out_channels=64):
        ...
        # 48*256
        self.encoder3 = EncoderBottleneck_DA(out_channels * 4, out_channels * 8)
        # 24*512
    ...
    def forward(self, x):
        ...
        x3,da_layer = self.encoder3(x2)
        #  torch.Size([16, 512, 24, 24])
        x4 = self.encoder4(x3)
        x5 = self.spp(x4)
        x5 = self.spp_double_conv(x5)
        # layer=self.concat_layer(da_layer,trans_layer)
        return x0,x1,trans_layer,da_layer,x5
...
class Decoder(nn.Module):
    ...
    def forward(self, x0,x1,trans_layer,da_layer,x5):
        # 48*48
        # print(x5.shape,print(x3.shape))
        x = self.decoder1(x5, trans_layer)
        # 96*96
        x = self.decoder2(x, da_layer)
        # 184*184
...
class DA_TransUnet_SPP3(nn.Module):
    ...
    def forward(self, x):
        x0,x1,trans_layer,da_layer,x5 = self.encoder(x)
        x = self.decoder(x0,x1,trans_layer,da_layer,x5)
