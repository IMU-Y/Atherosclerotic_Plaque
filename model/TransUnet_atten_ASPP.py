...
class Encoder(nn.Module):
    def __init__(self, in_channels, out_channels=64):
        ...
        self.aspp = ASPP(out_channels * 16, out_channels * 16*4, 5, conv=self.conv, norm=self.norm)  # 金字塔池化层
        self.aspp_double_conv = DoubleConv(out_channels * 16 * 4, out_channels * 8)
...
class DecoderBottleneck_atten(nn.Module):
    def __init__(self, in_channels, out_channels, bilinear=True):
        super().__init__()
        self.atten = Attention_block(F_g =in_channels//2, F_l = in_channels//2, F_int = out_channels)
        if bilinear:
            self.upsample = nn.Upsample(scale_factor=2,mode='bilinear', align_corners=True)
            self.conv = DoubleConv(in_channels, out_channels, in_channels // 2)
        else:
...
class Decoder(nn.Module):
    def __init__(self, out_put_channels):
        ...
        self.decoder1 = DecoderBottleneck_atten(1024, 256)
...
class TransUnet_atten_ASPP(nn.Module):
