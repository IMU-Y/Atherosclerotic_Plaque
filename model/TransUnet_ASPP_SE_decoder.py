...
class Encoder(nn.Module):
    def __init__(self, in_channels, out_channels=64):
        ...
        # self.se = SE_block(out_channels * 8)
    ...
    def forward(self, x):
        ...
        # res_x3 = x3+self.se(x3)
        x4 = self.aspp(x3)
        x5 = self.aspp_double_conv(x4)
        x5 = self.conv1(x5)
        return x0,x1,x2,da_layer4,x5

# class DecoderBottleneck_SE(nn.Module):
...
class TransUnet_ASPP_SE_decoder(nn.Module):
