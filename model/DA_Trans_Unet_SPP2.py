...
class EncoderBottleneck_Trans(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.vit =  ViT(img_dim=384, in_channels=64, embedding_dim=128, head_num=8, mlp_dim=128,
...
class EncoderBottleneck_DA(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.da = DAmoudle(in_channels)
        self.maxpool_conv = nn.Sequential(
...
#     def __init__(self,in_channels,out_channels):
#         super().__init__()
#         self.zip = DoubleConv(in_channels, out_channels)
#
...
class fuse_layers(nn.Module):
    ...
    def forward(self, feat_p, trans):
        feat_c = self.conv_c2(trans)
...
class Encoder(nn.Module):
    def __init__(self, in_channels, out_channels=64):
        ...
        # 24*512
        self.encoder4 = EncoderBottleneck_DA(out_channels * 8, out_channels * 16)
        self.relu = nn.ReLU(inplace=True)
        self.spp = SPPLayer(out_channels * 16, 3)  # 金字塔池化层
        self.spp_double_conv = DoubleConv(out_channels * 16 * 4, out_channels * 8)
        # self.concat_layer=concat_layer(out_channels * 16, out_channels * 8)
        self.fuse_layers = fuse_layers(out_channels * 8)
    ...
    def forward(self, x):
        ...
        x4,da_layer = self.encoder4(x3)
        x5 = self.spp(x4)
        x5 = self.spp_double_conv(x5)
        layer=self.fuse_layers(da_layer,trans_layer)
        return x0,x1,x2,layer,x5
...
class Decoder(nn.Module):
    ...
    def forward(self, x0,x1,x2,layer,x5):
        # 48*48
        # print(x5.shape,print(x3.shape))
        x = self.decoder1(x5, layer)
        # 96*96
...
class DA_Trans_Unet_SPP2(nn.Module):
    ...
    def forward(self, x):
        x0,x1,x2,layer,x5 = self.encoder(x)
        x = self.decoder(x0,x1,x2,layer,x5)
