...
class DAmoudle(nn.Module):
    def __init__(self,in_channels,norm_layer=nn.BatchNorm2d, norm_kwargs=None, **kwargs):
        ...
        self.pos = _PositionAttentionModule(inter_channels, **kwargs)
        self.cha = _ChannelAttentionModule(**kwargs)
...
class Encoder(nn.Module):
    def __init__(self, in_channels, out_channels=32):
        ...
        self.spp_double_conv = DoubleConv(out_channels * 16 * 4, out_channels * 16)
    ...
    def forward(self, x):
        ...
        x2,da_layer1 = self.encoder2(x1)
        # torch.Size([16, 256, 48, 48])
        x3,da_layer2 = self.encoder3(x2)
        #  torch.Size([16, 512, 24, 24])
        x4,da_layer3 = self.encoder4(x3)
        x5 = self.spp(x4)
        ...
        x5 = self.relu(x5)
        return da_layer1,da_layer2,da_layer3,x5
...
class Decoder(nn.Module):
    ...
    def forward(self, da_layer1,da_layer2,da_layer3,x5):
        # 48*48
        # print(x5.shape,print(x3.shape))
        x = self.decoder1(x5, da_layer3)
        # 96*96
        x = self.decoder2(x, da_layer2)
        # 184*184
        x = self.decoder3(x, da_layer1)
        x = self.decoder4(x)
...
class DA_SPP_UNet(nn.Module):
    ...
    def forward(self, x):
        da_layer1,da_layer2,da_layer3,x5 = self.encoder(x)
        x = self.decoder(da_layer1,da_layer2,da_layer3,x5
                         )
