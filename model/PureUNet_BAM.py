...
class PureUNet_BAM(nn.Module):
    def __init__(self, in_channels=3, output_channels=2, bilinear=True):
        ...
        self.bam1 = BAM(512 // factor)

        self.up2 = Up(512, 256 // factor, bilinear)
        self.bam2 = BAM(256 // factor)

        self.up3 = Up(256, 128 // factor, bilinear)
        self.bam3 = BAM(128 // factor)
    ...
    def forward(self, x):
        ...
        x = self.bam1(self.up1(x5, x3))
        # print(x.shape)
        # self.features.append(x)
        x = self.bam2(self.up2(x, x2))
        # self.features.append(x)
        x = self.bam3(self.up3(x, x1))
        # self.features.append(x)
        x = self.up4(x, x0)
